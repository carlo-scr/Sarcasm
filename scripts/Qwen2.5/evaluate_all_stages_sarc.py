"""
Comprehensive evaluation script to compare all training stages on SARC dataset:
1. Base Model (zero-shot)
2. After SFT (Phase 1)
3. After DPO (Phase 2)

Uses a fixed seed and 500 balanced samples from SARC test set for reproducibility.
"""
...
def evaluate_model(model, tokenizer, df, model_name, sample_size=None):
    ...
    if sample_size:
        df = df.sample(n=min(sample_size, len(df)), random_state=EVAL_SEED)
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc=model_name):
        text = row['comment']  # SARC dataset uses 'comment' column
        true_label = int(row['label'])  # SARC dataset uses 'label' column
...
def main():
    # Load test dataset
    print("="*70)
    print("COMPARATIVE MODEL EVALUATION ON SARC DATASET")
    print("="*70)
    
    test_csv_path = 'data/SARC/train-balanced-sarcasm.csv'
    print(f"\nLoading SARC dataset from: {test_csv_path}")
    
    if not os.path.exists(test_csv_path):
        print(f"❌ Dataset not found at {test_csv_path}")
        return
    ...
    sarc_df = df_test[df_test['label'] == 1]
    notsarc_df = df_test[df_test['label'] == 0]
    ...
    df_test = pd.concat([sarc_sample, notsarc_sample]).sample(frac=1, random_state=EVAL_SEED).reset_index(drop=True)
    print(f"✓ Sampled {len(df_test)} examples for evaluation (balanced, seed={EVAL_SEED})")
    
    # Count classes
    sarc_count = (df_test['label'] == 1).sum()
    notsarc_count = (df_test['label'] == 0).sum()
    ...
    # Stage 3: After DPO
    print(f"\n{'='*70}")
    print("STAGE 3: After DPO (Phase 2 - Preference refinement)")
    print("="*70)
    ...
    if os.path.exists(dpo_path):
        try:
            ...
            results_dpo, _ = evaluate_model(model, tokenizer, df_test, "DPO Model (Mistakes)")
        ...
        print("    Run 'python dpo_train_v2.py' first")
    
    # Summary comparison
    print("\n" + "="*70)
    print("COMPARATIVE RESULTS SUMMARY (SARC TEST SET)")
    print("="*70)
    ...
    if len(all_results) > 0:
        ...
        comparison = {
            ...
            'test_set': 'SARC train-balanced-sarcasm.csv',
        ...
        with open('comparative_results_sarc.json', 'w') as f:
            json.dump(comparison, f, indent=2)
        
        print(f"\n✓ Results saved to comparative_results_sarc.json")
    else:
