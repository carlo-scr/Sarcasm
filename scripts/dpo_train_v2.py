...
def train_dpo_with_config(
    ...
    Args:
        ...
        sample_size: Number of samples from iSarcasm to use
    ...
    if use_wandb:
        wandb.init(
            ...
            name=f"dpo-beta{beta}-{datetime.now().strftime('%Y%m%d-%H%M%S')}",
            config={
                "model": "Qwen2.5-0.5B-Instruct",
                "method": "DPO",
                "sft_model": sft_model_path,
                "dataset": "iSarcasm2022 (NEW, not used in SFT)",
                "beta": beta,
    ...
    # Create reference model (copy of SFT model, frozen)
    reference_model = deepcopy(model)
    reference_model.eval()
    for param in reference_model.parameters():
        param.requires_grad = False
    
    print("✓ Reference model created (frozen SFT model for KL tracking)")
    
    # Re-apply LoRA to model for DPO training
    ...
    # Load iSarcasm dataset for DPO (KEY FIX)
